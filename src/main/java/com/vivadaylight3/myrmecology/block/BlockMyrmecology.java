...
public class BlockMyrmecology extends BlockContainer {
	...
	else if (metadata == meta1 && side == 2 || metadata == meta2
		&& side == 5 || metadata == meta3 && side == 3
		|| metadata == meta4 && side == 4) return BlockSide.FRONT;
	else if (metadata == meta1 && side == 4 || metadata == meta2
		&& side == 2 || metadata == meta3 && side == 5
		|| metadata == meta4 && side == 3) return BlockSide.RIGHT;
	else if (metadata == meta1 && side == 5 || metadata == meta2
		&& side == 3 || metadata == meta3 && side == 4
    ...
    public void breakBlock(final World world, final int x, final int y,
	    final int z, final Block block, final int meta) {
	if (hasTileEntity) {
		...
		for (int i1 = 0; i1 < inv.getSizeInventory(); ++i1) {
			...
			for (final float f2 = rand.nextFloat() * 0.8F + 0.1F; itemstack.stackSize > 0; world
			    ...
			    entityitem = new EntityItem(world, x + f, y + f1, z
				    + f2, new ItemStack(itemstack.getItem(),
			    ...
			    if (itemstack.hasTagCompound()) entityitem
				    .getEntityItem().setTagCompound(
	...
	if (guiID >= 0) if (!world.isRemote && !player.isSneaking()) {
	...
	} else blockIcon = reg.registerIcon(uName);
    }
